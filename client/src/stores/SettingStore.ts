...
import { DEFAULT_OLLAMA_MODEL, DEFAULT_OPENAI_MODEL, OLLAMA_URL } from "../constants";
...
export class SettingStore {
  ...
  updateProvider(value: LlmProvider): void {
    this.provider = value;
    if (value === LlmProvider.OpenAI && this.llmName === DEFAULT_OLLAMA_MODEL) {
      this.llmName = DEFAULT_OPENAI_MODEL;
    }
    if (value === LlmProvider.Ollama && this.llmName === DEFAULT_OPENAI_MODEL) {
