...
export const ChatComposer = observer(() => {
	...
	const usagePercentage = hasActualUsage
		? Math.min(
				100,
				Math.round(
					((usage.promptEvalCount ?? 0) / MAX_TOKEN_CONTEXT) * 100,
				),
			)
		: 0;
	...
	return (
		...
		>
			...
			<div
				...
				title={
					hasActualUsage
						? "Reported by Ollama from the last completed request."
						: "Waiting for Ollama usage from the first completed request."
				}
			>
				<span>
					{hasActualUsage
				...
				<span>
					...
					{usage?.evalCount !== undefined
						? ` · Output ${usage.evalCount.toLocaleString()}`
						: ""}
				</span>
			</div>
