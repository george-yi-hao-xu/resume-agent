...
function normalizeUiPatch(value: unknown): UiPatch | null {
  ...
  if (action === PatchAction.UpdateCss && typeof patch.selector === "string" && isStringRecordLike(patch.styles)) {
    return {
      action,
  ...
  if (action === PatchAction.UpdateText && typeof patch.selector === "string" && patch.text !== undefined) {
    return {
      action,
      selector: patch.selector,
  ...
  if (action === PatchAction.InsertHtml) {
    const parent = typeof patch.parent === "string" ? patch.parent : typeof patch.selector === "string" ? patch.selector : "";
    if (parent && typeof patch.html === "string") {
      return {
        action,
        parent,
        position: typeof patch.position === "string" ? patch.position as InsertPosition : undefined,
        html: patch.html
      };
    }
  }

  if (action === PatchAction.RemoveElement && typeof patch.selector === "string") {
    return {
      action,
      selector: patch.selector
    };
  ...
  if (action === PatchAction.SetSectionLayout && patch.layout === "two_column") {
  ...
  if (action === PatchAction.ClonePage) {
    const sourcePage = typeof patch.sourcePage === "string" ? patch.sourcePage : typeof patch.source === "string" ? patch.source : "1";
    const targetPage = typeof patch.targetPage === "string" ? patch.targetPage : typeof patch.target === "string" ? patch.target : "";
    ...
    if (targetPage) {
      return {
        ...
        sourcePage,
        targetPage,
        targetLanguage: typeof patch.targetLanguage === "string" ? patch.targetLanguage : undefined,
        textUpdates
      };
    }
  }
...
function normalizeResumeSectionArray(value: unknown): Array<"summary" | "experience" | "skills" | "projects"> | null {
  ...
  const sections = values.filter((item): item is "summary" | "experience" | "skills" | "projects" => {
...
function normalizeClonePageTextUpdates(value: unknown): Array<{ selector: string; text: string }> | undefined {
  ...
  const updates = value
    ...
    .filter((item) => typeof item.selector === "string" && item.text !== undefined)
    .map((item) => ({
...
function stringifyRecord(value: Record<string, unknown>): Record<string, string> {
  return Object.fromEntries(
    Object.entries(value)
      .filter(([property, propertyValue]) => property && propertyValue !== undefined && propertyValue !== null)
