...
describe("server LlmService", () => {
	...
	it("normalizes Ollama API URLs", () => {
		expect(
			getOllamaTagsUrl("http://localhost:11434/api/chat?x=1#hash"),
		).toBe("http://localhost:11434/api/tags");
		expect(getOllamaChatUrl("http://localhost:11434")).toBe(
			"http://localhost:11434/api/chat",
		);
		expect(getOllamaChatUrl("http://localhost:11434/api/tags")).toBe(
			"http://localhost:11434/api/chat",
		);
	});
	...
	it("delegates patch generation to PatchWorkflowService", async () => {
		...
		await expect(
			service.getPatchesFromInstruction(request, "request-delegate"),
		).resolves.toEqual({
			patches: [],
			provider: LlmProvider.Ollama,
			model: "qwen2.5-coder:7b",
		});
		expect(patchWorkflowService.run).toHaveBeenCalledWith(
			request,
			"request-delegate",
		);
	});
		...
		const result = await service.getPatchesFromInstruction(
			{
				...
				resumeDom:
					'<main data-resume-root><article class="resume">Full DOM text</article></main>',
			},
			"request-1",
		);
	...
	it("includes full DOM for page translation and duplication requests", async () => {
		...
		expect(body.messages[0].content).toContain(
			"prefer clone_page over insert_html",
		);
		expect(body.messages[0].content).toContain(
			"clone_page may also refresh an existing target page",
		);
		expect(body.messages[0].content).toContain(
			"include textUpdates inside clone_page",
		);
		expect(body.messages[0].content).toContain(
			'parent "[data-resume-root]"',
		);
		expect(body.messages[0].content).toContain(
			"copy the source page DOM tree deeply",
		);
		expect(body.messages[0].content).toContain(
			"Never replace a non-empty source container with an empty target container",
		);
	...
	it("ignores invalid model patches instead of failing the whole request", async () => {
		...
		await expect(
			service.getPatchesFromInstruction(
				{
					instruction: "Change title",
					resumeSummary: "Page 1",
				},
	...
	it("logs failed patch workflow requests", async () => {
		...
		await expect(
			...
			"Ollama returned 500 from http://localhost:11434/api/chat.",
		);
...
function createConfig(
	overrides: Partial<ReturnType<LlmConfig["getRuntimeConfig"]>> = {},
): LlmConfig {
	return {
		getRuntimeConfig: () => ({
			provider: LlmProvider.Ollama,
