...
import { buildPatchModelMessages, shouldIncludeFullDom } from "./llm.prompt-builder";
...
export function normalizePatchRequestStep(state: PatchWorkflowState): PatchWorkflowState {
  return {
    ...state,
    resumeSummary: state.request.resumeSummary ?? state.request.resumeStructure ?? "",
    resumeDom: state.request.resumeDom ?? ""
  };
}

export function selectPatchContextStep(state: PatchWorkflowState): PatchWorkflowState {
  return {
    ...state,
    usedFullDom: shouldIncludeFullDom(state.request.instruction, state.request.conversationHistory ?? [])
  };
}

export function buildPatchMessagesStep(state: PatchWorkflowState): PatchWorkflowState {
  const resumeSummary = state.resumeSummary ?? state.request.resumeSummary ?? state.request.resumeStructure ?? "";
...
export async function callPatchModelStep(
  ...
  const result = await context.providerService.callRaw(context.config, requireMessages(state));
...
export function parsePatchOutputStep(state: PatchWorkflowState): PatchWorkflowState {
