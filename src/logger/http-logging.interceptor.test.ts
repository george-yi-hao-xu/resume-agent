...
describe("HttpLoggingInterceptor", () => {
  it("logs completed HTTP requests", async () => {
    ...
    await expect(lastValueFrom(interceptor.intercept(context, createHandler(of({ ok: true }))))).resolves.toEqual({ ok: true });
  ...
  it("logs failed HTTP requests", async () => {
    ...
    await expect(
      lastValueFrom(interceptor.intercept(
        createHttpContext({ method: "GET", originalUrl: "/api/llm/status", headers: {} }, { statusCode: 500 }),
        createHandler(throwError(() => error))
      ))
    ).rejects.toThrow("Boom");

    expect(logger.error).toHaveBeenCalledWith("http_request_failed", expect.objectContaining({
      method: "GET",
      url: "/api/llm/status",
      statusCode: 503,
      error: "Boom",
