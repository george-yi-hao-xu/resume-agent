...
describe("StructuredLogger", () => {
  ...
  it("can mirror only errors to console", () => {
    ...
    expect(readJsonLines(logPath).map((entry) => entry.event)).toEqual(["info_event", "error_event"]);
  });
  ...
  it("falls back to console error when file writes fail", () => {
    ...
    expect(errorSpy).toHaveBeenCalledWith(expect.stringContaining("log_file_write_failed"));
  });
});
