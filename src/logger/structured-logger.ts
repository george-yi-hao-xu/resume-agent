...
function getLogFilePath(): string {
	const configuredPath =
		process.env.LOG_FILE_PATH?.trim() || DEFAULT_LOG_FILE_PATH;
...
function getEventLogFilePath(appLogFilePath: string, event: string): string {
	...
	return join(
		dirname(appLogFilePath),
		"events",
		`${sanitizeEventName(event)}${extension}`,
	);
}
...
function getRotatedLogPath(filePath: string, index: number): string {
	...
	return join(
		dirname(filePath),
		`${basename(filePath, extension)}.${index}${extension}`,
	);
}
